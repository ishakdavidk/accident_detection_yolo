...
def main():
    ...
    video_path = '../dataset/yt_dashcam_accident/accident2.mp4'  # or 0 for webcam

    # <<< NEW: output path / enable flag
    save_output = True
    output_path = "output_accident_detect.mp4"
    ...
    # <<< NEW: prepare VideoWriter using original frame size
    ...
    if save_output:
        ...
        if not out.isOpened():
            print("WARNING: Failed to open VideoWriter, disabling save_output.")
            save_output = False
    ...
    try:
        while True:
            ...
            cv2.putText(
                frame, last_label + " - " + prob_label, (x, y),
                cv2.FONT_HERSHEY_SIMPLEX, 1, last_color, 2, cv2.LINE_AA
            )

            # <<< NEW: write the *annotated* original-size frame to output video
            if save_output and out is not None:
                out.write(frame)

            # Info panel (for visualization only)
            ...
            for i, line in enumerate(info_lines):
                y_line = y0 + i * dy
                cv2.putText(
                    info_image, line, (10, y_line),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 1, cv2.LINE_AA
                )

            # Show (resize only for display)
        ...
        print("Done. Saved to:", output_path if save_output else "no file saved")
